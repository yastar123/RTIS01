...
type Section =
  "overview" | "reservations" | "screening" | "questions" | "tutorials" | "articles" | "users";
...
function AdminPage() {
  ...
  const navItems: { key: Section; label: string; icon: typeof LayoutDashboard }[] = [
    ...
    { key: "questions", label: "Soal Skrining TCM", icon: HelpCircle },
        ...
        <main className="min-w-0 flex-1 w-full p-3 sm:p-5 lg:p-6">
                ...
                {section === "overview"
                  ...
                  : section === "reservations"
                    ...
                    : section === "screening"
                      ...
                      : section === "questions"
                        ? "Kelola Soal Skrining TCM & Anamnesis"
                        : section === "articles"
          ...
          {isLoading ? (
            ...
            <>
              ...
              {section === "questions" && <QuestionnaireManagerSection onNavigate={setSection} />}
              {section === "articles" && (
                <ArticleSection
                  articles={articles}
              ...
              {section === "tutorials" && <TutorialSection onNavigate={setSection} />}
              {section === "users" && (
                <UserSection
                  users={filteredUsers}
                  query={userQuery}
...
function ScreeningSection({
  ...
  const handleShareWhatsApp = (item: AdminScreeningItem) => {
    ...
    const encodedPayload = encodeURIComponent(
      btoa(encodeURIComponent(JSON.stringify(resultPayload))),
    );
          ...
          <div className="space-y-3">
            ...
            {isLoadingReport ? (
              <div className="rounded-xl border p-8 text-center space-y-2 bg-card">
                ...
                <p className="text-xs text-muted-foreground">
                  Sistem sedang menganalisis pola sindrom TCM, formulasi herbal, dan rekomendasi
                  titik akupunktur pasien...
                </p>
              </div>
            ) : aiError ? (
...
function TutorialSection({ onNavigate }: { onNavigate: (section: Section) => void }) {
...
function TutorialDialog({
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      await adminFetch(
        value === "new" ? "/api/admin/tutorials" : `/api/admin/tutorials/${value.id}`,
        {
          method: value === "new" ? "POST" : "PATCH",
          body: JSON.stringify({
                  ...
                  <div>
                    <p className="text-xs font-semibold text-foreground">
                      {isReading
                        ? "Sedang memproses..."
                        : fileName ||
                          `Klik untuk upload ${form.mediaType === "video" ? "Video (MP4, WEBM)" : "Gambar (JPG, PNG, WEBP)"}`}
                    </p>
...
/* =========================================================================
   TCM SCREENING QUESTIONNAIRE CMS (BAGIAN A-L CRUD MANAGER)
   ========================================================================= */

function QuestionnaireManagerSection({ onNavigate }: { onNavigate?: (section: Section) => void }) {
  ...
  const normalizeSecList = (list: any[]): TcmQuestionSection[] => {
    return list.map((s, idx) => {
      const secId = s.id || s.key || s.code || `sec_${idx}`;
      const secLetter = s.letter || s.code || String.fromCharCode(65 + idx);
      return {
        ...s,
        id: secId,
        key: s.key || secId,
        code: s.code || secLetter,
        letter: secLetter,
        title: s.title || `${secLetter}. BAGIAN ${idx + 1}`,
        fields: (s.fields || []).map((f: any, fIdx: number) => ({
          ...f,
          id: f.id || `f_${secId}_${fIdx + 1}`,
          number: f.number ?? fIdx + 1,
...
/* =========================================================================
   QUESTION FIELD CRUD MODAL DIALOG
   ========================================================================= */
...
function QuestionFieldDialog({
  ...
  // Auto generate field ID from label if new and fieldId empty
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Parse options by lines or commas
